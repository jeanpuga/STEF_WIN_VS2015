...
#include "Face.h"
int expres=0;
Face::Face(GLint altura, GLint largura, GLint etinia, Grafo g) {
	//pFile = fopen ("OutrasFaces.txt","w");
	this->altura = altura;
	...
	medidas = new Medidas();
	antropometria = new Antropometria(altura, largura, etinia, g);
...
void Face::inicilizaArraysCurvas() {
	...
	//
	antropometria->getContornoQueixo(queixo);
	//
...
void Face::desenhaContornoEsquerdo() {
	imprimePontos("Contorno cima", cec, 7);
	//desenha->desenhaCurva(cec, 4);
	imprimePontos("Contorno esquerdo baixo", ceb, 4);
	//desenha->desenhaCurva(ceb, 4);
	imprimePontos("Contorno queixo", queixo, 3);

}

void Face::desenhaContornoDireito() {
	//imprimePontos("Contorno direito cima", cdc, 7);
...
void Face::desenhaPupilaEsquerda() {
	imprimePontos("wPupila esquerda", pue, 1);
	//desenha->desenhaCirculo(pue, pupilaR);
}

void Face::desenhaPupilaDireita() {
	imprimePontos("wPupila direita", pud, 1);
...
void Face::imprimePontos(const char* contexto, GLfloat pontos[][3],
		GLint nPontos) {

	fprintf(pFile, "Curva %s:{\n", contexto);
	GLint i;
	for (i = 0; i < nPontos; i++) {
...
void Face::satisfacao(GLfloat intensidade) {
	...
	movimentos->au6CheekRaiserLidCompressor(oeb, oec, odb, odc, pe, pd, roec,
			roem, roeb, rodc, rodm, rodb, intensidade);
    //movimentos->movAlpha(lsc, lsb, lic, lib, oeb, oec, odb, odc, pe, pd, roec,
			//roem, roeb, rodc, rodm, rodb, intensidade,expres);
...
void Face::tristeza(GLfloat intensidade) {
	...
	imprimePontos("Auxiliares", aux, 2);
	//FAZER AU11
}
void Face::neutra() {
	GLfloat aux[7][3];
...
void Face::surpresa(GLfloat intensidade) {
	movimentos->au1au2(se, sd, oec, odc, rtc, rtm, rtb, rctc, rctm, rctb,
			intensidade);
	desenhaRugasTesta(intensidade);
	movimentos->au5UpperLidRaiser(oeb, oec, odb, odc, pe, pd,
			0.8 * intensidade);
	movimentos->au26JawDrop(lsc, lsb, lic, lib, intensidade);
}

void Face::medo(GLfloat intensidade) {
	movimentos->au1au2au4(se, sd, oec, odc, rtc, rtm, rtb, rctc, rctm, rctb,
			intensidade);
